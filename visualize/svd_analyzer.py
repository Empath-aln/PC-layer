...
_WEIGHT_KEY_TO_PC_FLAG = {
    ...
    'w1': 'precondition_ffn', 'w2': 'precondition_ffn', 'w3': 'precondition_ffn',
}
...
def extract_weight_matrices(model) -> Dict[str, torch.Tensor]:
    ...
    - layers.{i}.attention.wq/wk/wv/wo, layers.{i}.feed_forward.w1/w2/w3
...
def extract_sn_norms(model) -> Dict[str, torch.Tensor]:
    """Extract spectral norms (sn_norm) from PCLinear modules.

    Mirrors training-time / forward-time behaviour: first refresh the persistent
    sn_u/sn_v buffers from the loaded checkpoint weights via
    update_model_sn_state() (power iteration), then read back each module's
    spectral norm with _compute_sn_norm_from_state() — exactly what
    PCLinear.forward() does before applying the preconditioner.

    Only collects modules where pc_norm_type == 'sn'.

    Args:
        model: PyTorch model with loaded checkpoint weights

    Returns:
        Dict mapping module names to sn_norm scalar tensors. Keys match the
        layer_key produced by extract_weight_matrices (the module path with the
        .linear.weight suffix stripped).
    """
    from torchtitan.pc_layer.pc_layer import PCLinear, update_model_sn_state

    # Refresh sn_u/sn_v from the loaded weights (consistent with training).
    update_model_sn_state(model)

    sn_norms = {}
    for module_name, module in model.named_modules():
        if not isinstance(module, PCLinear) or not module._uses_sn_norm():
            continue
        with torch.no_grad():
            sn_norms[module_name] = module._compute_sn_norm_from_state(module.linear.weight)

    return sn_norms


def apply_pc_transform(weight: torch.Tensor, model_config, sn_norm=None, return_norm=False):
    ...
    Args:
        ...
        sn_norm: Optional pre-computed spectral norm (required when pc_norm_type == 'sn')
    ...
    return pc(weight, sn_norm=sn_norm, return_norm=return_norm)


def compute_svd_with_pc(weights: Dict[str, torch.Tensor], model_config,
                        step: Optional[int] = None,
                        sn_norms: Optional[Dict[str, torch.Tensor]] = None) -> Dict[str, Dict[str, Any]]:
    ...
    for name, weight in tqdm(weights.items(), desc=desc, unit="layer"):
        ...
        if _should_apply_pc(name, model_config):
            # For sn norm: use the spectral norm pre-computed from PCLinear's
            # sn_u/sn_v state (mirrors PCLinear.forward).
            sn_norm_val = sn_norms.get(name) if sn_norms else None

            # return_norm=True: W_norm is the spectral norm used for normalization.
            pc_weight, W_norm = apply_pc_transform(
                weight, model_config, sn_norm=sn_norm_val, return_norm=True
            )
...
def has_any_pc_enabled(model_config) -> bool:
    ...
    pc_flags = ['precondition_ffn', 'precondition_o',
                'precondition_qk', 'precondition_v']
...
def build_val_dataloader(job_config, tokenizer) -> Optional[Any]:
    ...
    try:
        data_loader = build_hf_data_loader(
            getattr(job_config.training, 'dataset', ''),
            dataset_val_path,
            tokenizer,
            batch_size,
            seq_len,
...
def get_output_dir(job_config) -> str:
    ...
    Format: ./visualization_output/svd/{model.name}_{model.flavor}_{optimizer.name}_{metrics.swanlab_comment}
    SVD dumps live under a dedicated svd/ subdir so many runs don't clutter the
    visualization_output root (plot outputs sit alongside it).
    """
    model_name = job_config.model.name
    model_flavor = job_config.model.flavor
    optimizer_name = job_config.optimizer.name
    swanlab_comment = job_config.metrics.swanlab_comment

    dir_name = f"{model_name}_{model_flavor}_{optimizer_name}_{swanlab_comment}"
    return f"./visualization_output/svd/{dir_name}"
...
def parse_visualize_steps(step_config: str, available_steps: List[int]) -> List[int]:
    ...
    try:
        ...
        raise ValueError(f"Invalid visualize.step format: '{step_config}'. "
                         "Use -1 for all steps or comma-separated list like '1,2000,60000'")
...
def run_visualize(job_config) -> List[str]:
    ...
    to separate JSON files per step.
    ...
    checkpoint_path = (
        ...
        f"{job_config.metrics.swanlab_comment}"
    )
    ...
    for step in steps_to_process:
        ...
        if val_dataloader is not None:
            try:
                ...
                logger.info(f"[Checkpoint Verification] Step {step} - "
                            f"Val loss: {val_loss:.4f} (over {num_val_batch} batches)")
            except Exception as e:
        ...
        if pc_enabled:
            ...
            sn_norms = extract_sn_norms(model)
            svd_results = compute_svd_with_pc(weights, job_config.model, step=step, sn_norms=sn_norms)
        else:
        ...
        if pc_enabled:
            metadata["pc_config"] = {
                "precondition_ffn": getattr(job_config.model, 'precondition_ffn', False),
                ...
                "pc_norm_type": getattr(job_config.model, 'pc_norm_type', 'sn'),
                "pc_level": getattr(job_config.model, 'pc_level', 0),
                "recover_w_norm": getattr(job_config.model, 'recover_w_norm', True),
                "learnable_gamma": getattr(job_config.model, 'learnable_gamma', False),
                "gamma_init_value": getattr(job_config.model, 'gamma_init_value', 1.0),
            }
