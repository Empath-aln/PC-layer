...
class ModelArgs:
    ...
    precondition_ffn: bool = False
    precondition_qk: bool = False
    precondition_v: bool = False
    precondition_o: bool = False
    power_iter: int = 5
    # PC norm type: "sn" (spectral norm), "none" (no normalization)
    pc_norm_type: Optional[str] = "sn"
...
class Attention(nn.Module):
    ...
    def __init__(self, model_args: ModelArgs, layer_id: int):
        ...
        # Create the original nn.Linear layers
        wq = nn.Linear(
        ...
        # Wrap nn.Linear with PCLinear (depending on the config)
    ...
    def init_weights(self, init_std: float):
        # Get the actual nn.Linear layer (may be a PCLinear or a plain Linear)
    ...
    def forward(
        ...
        # PCLinear automatically applies the preconditioner in forward, so just call it directly
        xv = self.wv(x)
...
class FeedForward(nn.Module):
    ...
    def __init__(
        ...
        # Wrap nn.Linear with PCLinear (depending on the config)
        self.w1 = PCLinear(w1, model_args, layer_id) if model_args.precondition_ffn else w1
        self.w2 = PCLinear(w2, model_args, layer_id) if model_args.precondition_ffn else w2
        self.w3 = PCLinear(w3, model_args, layer_id) if model_args.precondition_ffn else w3
        ...
        # Note: gamma parameters are now managed by PCLinear, no longer created here
